...
本文档保留原 Android memleak 的使用说明和历史真机验证记录。
当前构建入口已切换为独立 NDK 流程，见 [项目说明](../README.md)。
下文历史设备结果不代表本次 NDK 新产物已完成同样的真机验证。
本次独立 NDK 产物的实测结果另见 [2026-09-05 真机验证](device-validation-2026-09-05.md)。

## 适用范围

- 源码由脚本下载上游 release，生成于 `sources/`。
...
```sh
# 默认使用 /mnt/develop/android-ndk-r30 和 nproc 并行数。
make release
```
